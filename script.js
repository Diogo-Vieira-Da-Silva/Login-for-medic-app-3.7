...
const cadastro = [
  ...
  // Atualizar saudação com o nome do enfermeiro
  if (responsavel) {
      atualizarSaudacao(responsavel);
  }

  // Mudança: Atualizar saudação com o nome do enfermeiro após cadastro
  if (responsavel) {
      atualizarSaudacao(responsavel);
  }
...
// Mudança: Função para atualizar a saudação com o nome do enfermeiro
function atualizarSaudacao(nome) {
    const saudacaoElement = document.querySelector('.home-ola');
    if (nome) {
        const primeiroNome = nome.split(' ')[0]; // Pega o primeiro nome
        saudacaoElement.innerHTML = `<strong>Olá, Enfermeiro(a) ${primeiroNome}!</strong>`;
        localStorage.setItem('nomeEnfermeiro', nome);
    } else {
        saudacaoElement.innerHTML = `<strong>Olá, Enfermeiro(a)!</strong>`;
    }
}

// Mudança: Carregar saudação ao carregar a página
window.addEventListener('load', function() {
    const nomeSalvo = localStorage.getItem('nomeEnfermeiro');
    if (nomeSalvo) {
        atualizarSaudacao(nomeSalvo);
    }
});

// Mudança: Handler para o formulário de login
...
if (loginForm) {
    loginForm.addEventListener('submit', function(e) {
        ...
        if (nome) {
            ...
            showMessage('Por favor, digite o nome.', 'erro');
        }
    });
}
